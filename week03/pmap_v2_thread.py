...
def save_data():
    ...
    output = open(inputArgs["savePath"],mode= 'w',encoding='utf-8')
...
# par = functools.partial(get_ping_result,"10.60.99.32",80)
...
if __name__ == '__main__':
    ...
    if "ping" == inputArgs["testMethod"]:
        with ThreadPoolExecutor(n) as executor:
            ...
            # port = ["80"];
                # executor.map(partial(get_ping_result, ip_address = ip_address),port)
                executor.submit(get_ping_result, ip_address, 80)
            # t = threading.Thread(target = get_ping_result, args=(ip_address,80,))
            # t.start()
            # t.join()
        ...
        with ThreadPoolExecutor(n) as executor:
            ...
            # with ThreadPoolExecutor(n) as executor:
                for port in range(1024):
                    # t = threading.Thread(target = get_ping_result, args=(ip_address,port,))
                    # t.start()
                    executor.submit(get_ping_result,ip_address,port)
            # for port in range(1024):
            #     # with ThreadPoolExecutor(n) as executor:
            #         # port = [port]
            #         # executor.map(partial(get_ping_result, ip_address = ip_address),port)    
            #         # executor.submit(get_ping_result, ip_address, port)
            #     # list1 = [ip_address,port]
            #     # p.map(get_ping_result,list1)
            #     t = threading.Thread(target = get_ping_result, args=(ip_address,port,))
            #     t.start()
            #     t.join()

    print("ip_available:", ip_avalibale, "usedport", usedport)